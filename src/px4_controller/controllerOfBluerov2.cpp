...
void controllerOfBluerov2::controllLogic(Eigen::Vector3d &des_thrust, Eigen::Vector3d &des_rpy) {
//    double roll = 0.0;
//    double pitch = 0.0;
//    double yaw = 0.0;
//    double thrust_1 = -0.5;
...
//    double thrust_3 = 0.1;
    ...
    if (this->holdPosition) {
        ...
        des_thrust = getThrustForMavros(thrust12[0], thrust12[1], thrustHeight);

        des_rpy[0]= this->holdRoll;
        des_rpy[1]= this->holdPitch;
        des_rpy[2]= this->holdYaw;
        ...
        des_thrust[0] = this->desiredXThrustBody;
        des_thrust[1] = this->desiredYThrustBody;
        des_thrust[2] = thrustHeight;

        des_thrust = currentRotation*des_thrust;
        des_thrust = getThrustForMavros(this->desiredXThrustBody, this->desiredYThrustBody,thrustHeight);

        des_rpy[0]= this->desiredRoll;
        des_rpy[1]= this->desiredPitch;
        des_rpy[2]= this->desiredYaw;
    }
}
...
void controllerOfBluerov2::desiredStateCallback(const bluerov2commonmsgs::msg::DesiredStateForRobot::SharedPtr msg) {
...
void controllerOfBluerov2::getRollPitchYawThrustControllLogic(Eigen::Vector3d &des_thrust, Eigen::Vector3d &des_rpy) {
    this->controllLogic(des_thrust, des_rpy);

    //send visualization stuff
    ...
    p.x=position.x()+des_thrust.x();
    p.y=position.y()+des_thrust.y();
    p.z=position.z()+des_thrust.z();
    marker.points.push_back(p);
